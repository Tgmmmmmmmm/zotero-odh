...
class encn_LDOCE6MDX {
  ...
  async findLDOCE6(word) {
    ...
    for (const entry of allentries) {
      ...
      let expression =
        T(header.querySelector(".hwd")) || T(header.querySelector(".phrvbhwd")); //headword
        ...
        for (const subsense of subsenses) {
          ...
          if (examples.length > 0 && this.maxexample > 0) {
            ...
            for (const [index, example] of examples.entries()) {
              ...
              if (
                soundlink &&
                soundlink.getAttribute("href").indexOf("sound://") != -1
              )
                soundlink = putSoundTag(
                  base + soundlink.getAttribute("href").substring(8),
                ); // 8 = 'sound://'.length
