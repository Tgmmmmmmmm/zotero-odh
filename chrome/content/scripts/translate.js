Zotero.zoteroodh.translate = {
  ...
  callTranslate: async function (currentReader, force = false) {
    ...
    if (
      !force &&
      (!text.replace(/[\r\n]/g, "").replace(/\s+/g, "") ||
        Zotero.zoteroodh._sourceText === text)
    ) {
      Zotero.zoteroodh.view.updateResults();
    ...
    Zotero.zoteroodh.translate._timestamp = t;
    Zotero.debug(`zoteroodh: Translate ${t} start.`);

    let success = await Zotero.zoteroodh.translate.getTranslation();

    Zotero.debug(`zoteroodh: Translate ${t} returns ${success}`);
    if (Zotero.zoteroodh.translate._timestamp > t) {
      Zotero.debug(`zoteroodh: Translate ${t} overwritten.`);
      return true;
    }

    let enablePopup = Zotero.Prefs.get("zoteroodh.enablePopup");
    if (enablePopup && Zotero.zoteroodh.view.popupTextBox) {
      Zotero.zoteroodh.view.popupTextBox.remove();
  ...
  callTranslateAnnotation: async function (item) {
    ...
    if (
      Zotero.Prefs.get("zoteroodh.enableComment") &&
      !disable &&
      item.isAnnotation() &&
      ...
      if (Zotero.zoteroodh._sourceText != item.annotationText) {
        Zotero.zoteroodh._sourceText = item.annotationText;
        let success =
          await Zotero.zoteroodh.translate.getTranslation();
        if (!success) {
  ...
  callTranslateNote: async function (annotations) {
    try {
      for (let annotation of annotations) {
        if (Zotero.zoteroodh._sourceText !== annotation.text) {
          Zotero.zoteroodh._sourceText = annotation.text;
          await Zotero.zoteroodh.translate.getTranslation();
        }
        annotation.text = `${Zotero.zoteroodh._sourceText}\n----\n${Zotero.zoteroodh._translatedText}\n`;
      }
    } catch (e) {
      Zotero.debug(`zoteroodh.callTranslateNote Error: ${e}`);
    }
    Zotero.zoteroodh.translate._enableNote = false;
    Zotero.debug(`zoteroodh.callTranslateNote : ${annotations}`);
    return annotations;
  },
  ...
  getTranslation: async function () {
    ...
    return await Zotero.zoteroodh.translate[translateSource]();
  },
  ...
  */
  tencent: async function () {
    ...
    return await this.requestTranslate(
      ...
      (xhr) => {
        ...
        Zotero.debug(tgt);
        Zotero.zoteroodh._translatedText = tgt;
        return true;
      }
    );
  },
  ...
  getArgs: function () {
    ...
    if (typeof secret === "undefined") {
      secret = Zotero.zoteroodh.defaultSecret["caiyun"];
    }
    let sl = Zotero.Prefs.get("zoteroodh.sourceLanguage");
    if (typeof sl === "undefined") {
      sl = Zotero.zoteroodh.defaultSourceLanguage;
    }
    let tl = Zotero.Prefs.get("zoteroodh.targetLanguage");
    if (typeof tl === "undefined") {
      tl = Zotero.zoteroodh.defaultTargetLanguage;
    }
    let text = Zotero.zoteroodh._sourceText.replace(/\n/g, " ");
    return {
      secret,
      sl,
      tl,
...
Report issue here: https://github.com/windingwind/zotero-pdf-translate/issues
  ...
  requestTranslate: async function (request_func, parse_func) {
    ...
    Zotero.zoteroodh._debug = this.getErrorInfo("request");
    return false;
  },
  sources: [
    "googleapi",
